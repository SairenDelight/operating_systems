...
void * thread_producer(void * id){
    ...
    counter++;
    // printf("Finished running thread producer: %d\n", i);
...
void * thread_consumer(void * id){
    ...
    counter++;
    // printf("Finished running thread consumer: %d\n", i);
    pthread_mutex_unlock(&lock);
}
...
int main(int argc, char **argv)
{
    ...
    pthread_t producers_id[producer_count]; //initialize the producers
    pthread_t consumers_id[consumer_count]; //initialize the conumsers
    ...
    while(1){
        ...
        if(counter >= producer_count){
            ...
            pthread_mutex_unlock(&lock);
            break;
        } else if(pthread_create(&producers_id[counter], NULL, thread_producer, &counter) != 0){
    ...
    while(1){
        if(c >= producer_count)
            break;
        pthread_join(producers_id[c], NULL);
        c++;
    }
    ...
    while(1){
        ...
        if(counter >= consumer_count){
            ...
            pthread_mutex_unlock(&lock);
            break;
        } else if(pthread_create(&consumers_id[counter], NULL, thread_consumer, &counter) != 0){
            perror("Thread creation");
            exit(EXIT_FAILURE);
        }
    ...
    c = 0;
    do {
        pthread_join(consumers_id[c], NULL);
        c++;
